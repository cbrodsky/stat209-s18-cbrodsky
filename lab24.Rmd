...
```{r}
crimes_ca$narcotic_rate <- crimes_ca$narcotics_sum/crimes_ca$num_households*1000
ggplot(crimes_ca, aes(median_income, narcotic_rate))+
  geom_point(aes(label = area_number))
```

Thesis:  Generally speaking, the narcotic arrest rate does not change drastically as median income increases. There is a only a slight decrease in narcotic-based arrests as incomes increase. 
