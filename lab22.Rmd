...
**Answer**: 3rd = 369 and 97th = 4107.46
...
**Answer**: Perfectionism score (intercept) is larger than the normal and slow speeds approx by a factor of 3.
...
**Answer**: There is a statistically significant difference between normal and fast users, but not for slow users. 
...
**Answer**: Some people may finish quickly because they have simply answer the questions arbitarily without paying attention to the questions. Some may finish slowly because they take their time and really think about themselves in relation to each question. 
...
```{r}
model <- lm_basic(openness_to_change ~ 1 + apprehension + baseline, data = pf)
  reg_table(model, level = .9)
```
...
**Answer**: Compared with openness to change, there were statistically significant differences for both apprehension and the baseline. 

Take the previous model and add the variables `speed`, `gender`, and
`country`, the latter lumped into 5 categories.

```{r}
model <- lm_basic(openness_to_change ~ 1 + apprehension + baseline + speed + gender + fct_lump(country,5), data = pf)
  reg_table(model, level = .9)
```

Does this change the slopes for your two traits much?

**Answer**: No, neither slopes for apprehension or baseline changed much. Although it did decrease slightly for openness_to_change from -.1 to -.2.
