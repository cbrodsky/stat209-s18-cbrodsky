...
**Answer**: The mean here refers to the average proportion of crimes that resulted in an arrest in proportion to the area. The sum is the total number of crimes.  
...
**Answer**: This mean is the average proportion of crimes classified as theft.  
...
**Answer**: This value refers to the total number of thefts. 
...
**Answer**: This value indicates the number of thefts per 1000 households. 
...
**Answer**: This value is larger than the mean of the arrest flag variable. Proportionally, area 23 has more arrests than the entire dataset.  
...
**Answer**: This mean is smaller than the entire dataset. The value is proportionally less than the entire datset. 
...
**Answer**: This value is larger. This means there are more thefts per 1000 households in area 23 compared to the entire dataset area. 
...
```{r}
area_23 <- filter(ca, area_number == "23")
area_23$num_households
```
...
```{r}
crime_ca <- group_summarize(crimes, area_number)
```
...
```{r}
crimes_ca <- left_join(crime_ca, ca)
```
...
**Answer**: As the median income goes up, the theft rate generally goes down. There is one significant outlier where both the median income and theft rate are substantially high. 
...
ggplot(crimes_ca, aes(median_income, theft_rate))+
  geom_point()+
  geom_text(aes(label=area_number))
```
...
**Answer**: The area is high-end and largely urban making it an attractive target for robberies and thefts. In urban areas, bikes are commonly used and thus are easy targets for robbery. 
