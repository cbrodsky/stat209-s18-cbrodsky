...
```{r}
birds_new <- group_summarize(birds, type)
```
...
```{r}
ggplot(data=birds_new, aes(x=male_mass_median, y=egg_mass_median))+
  geom_point()+
  scale_x_log10()+
  scale_y_log10()
```
...
```{r}
birds_new_two <- filter(birds_new, n >= 5)
...
ggplot(data=birds_new, aes(x=male_mass_median, y=egg_mass_median))+
  geom_point()+
  geom_point(color = "red", data = birds_new_two)+
  scale_x_log10()+
  scale_y_log10()+
  annotate("text", x=650, y=2, label="Red Points Indicate Types With More Than 5 Birds")
